...
const Projects = () => {
    ...
    return (
        <div className='border-b lg:border-neutral-900 border-neutral-800' id='projects'>
            <motion.h1 initial={{ y: -50, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }} className='lg:mt-16 mt-12 lg:mb-18 mb-16 text-center text-4xl font-thin text-neutral-400 tracking-wider'>Projects</motion.h1>
            <div>
                {viewAll ? project && project.map((item, index) => {
                    return (
                        <div key={index} className='mb-8 flex flex-wrap lg:justify-center lg:gap-23'>
                            ...
                            <motion.div whileInView={{ opacity: 1, x: 0 }} initial={{ x: 100, opacity: 0 }} transition={{ duration: 0.7, delay: 0.5 }} className="w-full max-w-xl lg:w-3/4">
                                <Link to={`/project/${item._id}`}>
                                    <h6 className="mb-2 text-xl font-semibold bg-gradient-to-r from-neutral-300 to-stone-400 bg-clip-text text-transparent cursor-pointer tracking-wide">
                                        {item.title}
                                    </h6>
                                </Link>
                                <p className='mb-4 text-neutral-400 tracking-wide'>
                    ...
                    return (
                        <div key={index} className='mb-8 flex flex-wrap lg:justify-center lg:gap-20'>
                            <div className="w-full lg:w-1/4">
                                <Link to={`/project/${item._id}`}>
                                    <motion.img whileInView={{ opacity: 1, x: 0 }} initial={{ x: -100, opacity: 0 }} transition={{ duration: 0.7, delay: 0.5 }} className='rounded-md lg:mb-4 mb-6 lg:w-[100%] lg:mt-1 cursor-pointer border-1 border-neutral-800 shadow-lg shadow-black' src={item.projectBanner && item.projectBanner.url} alt="" />
                                </Link>
                ...
                {project && project.length > 6 && (
                    <div className='flex items-center justify-center'>
                        <button className='flex items-center justify-center px-4 py-4 bg-transparent text-neutral-300 rounded-full text-xl cursor-pointer gap-2 hover:gap-3 hover:transition-[5s] transition-[5s]' onClick={() => setViewAll(!viewAll)}>
                            {viewAll ? <><FaArrowLeft className='mt-1' /><p className='tracking-wide'>show less</p></> : <><p className='tracking-wide'>show more</p><FaArrowRight className='mt-1' /></>}
                        </button>
                    </div>
                )}
            </div>
